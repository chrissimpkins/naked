...
if sys.version_info[0] == 2 and sys.version_info[1] == 7: # do not run with tox in non-2.7 versions (not building C files and fails)
	...
	try:
		from Naked.toolshed.c.ink import Template, Renderer #test the C Ink template module
	except ImportError:
		os.system('naked build') #if the C modules are not compiled, compile them
		from Naked.toolshed.c.ink import Template, Renderer #then import
