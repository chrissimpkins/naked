...
if [ "$1" = "all" ];then
	"$TEST_COMMAND" "$NOSE_FLAGS" "$CAST_TESTS" "$COM_PARSE_TESTS" "$COM_ALL_COMMANDS" "$IO_TESTS" "$IO_TESTS_C" "$NETWORK_TESTS" "$NETWORK_TESTS_C" "$PYTHON_TESTS" "$SHELL_TESTS" "$SHELL_TESTS_C" "$STATE_TESTS" "$STATE_TESTS_C" "$SYS_TESTS" "$SYS_TESTS_C" "$TEMPLATE_TESTS" "$TEMPLATE_TESTS_C"
elif [ "$1" = "casts" ];then
